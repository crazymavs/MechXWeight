...
$(function () {
	...
	$('#elitePhoto').on('change', function (event) {
		let labelText = 'Upload your photo';
		if(this.files.length > 0){
			labelText = this.files[0].name;
		}
		$('#lblElitePhoto').text(labelText);
	});
});
...
$(function () {
	$('#tel').on('blur', function (e) { 
		...
		if($.trim(e.target.value) !== ''){
			$('.preloader').show();
			getUsersFromPhone(e.target.value);
		}
	});
	...
	// $('#feedbackTel').on('blur', function (e) { 
	// 	// e.preventDefault();
	// 	if($.trim(e.target.value) !== ''){
	// 		$('.preloader').show();
	// 		getUsersFromPhone(e.target.value);
	...
	// $("#frmEliteFeedback").on('submit', (function (e) {
    //     e.preventDefault();
	// 	saveFeedback();
    // }));
})